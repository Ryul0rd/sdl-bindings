...
var _img_load = _sdl_img.get_function[fn (UnsafePointer[UInt8]) -> UnsafePointer[_Surface]](
    "IMG_Load"
)


fn load_image(file: String) raises -> Surface:
    var _surface_ptr = _img_load(file.unsafe_ptr())
    if _surface_ptr:
        return Surface(_surface_ptr)
