struct Font:
    ...
    fn render_shaded(self, text: String, foreground: Color, background: Color) -> Surface:
        return Surface(render_shaded_text(self._font_ptr, text.unsafe_ptr(), foreground.as_uint32(), background.as_uint32()))
...
var _ttf_open_font = _sdl_ttf.get_function[fn(UnsafePointer[UInt8], Int32) -> UnsafePointer[C_Font]]('TTF_OpenFont')
fn ttf_font(name: UnsafePointer[UInt8], size: Int32) -> UnsafePointer[C_Font]:
    return _ttf_open_font(name, size)

var _ttf_close_font = _sdl_ttf.get_function[fn(UnsafePointer[C_Font]) -> None]('TTF_CloseFont')
