...
struct Surface[lif: AnyLifetime[False].type]:
    ...
    fn __init__(
        inout self, ref [lif]sdl: SDL, width: Int32, height: Int32
    ) raises:
        self.sdl = sdl
        self._surface_ptr = sdl._sdl.create_rgb_surface(
            0, width, height, 32, 0, 0, 0, 0
        )

    fn __init__(
        inout self, ref [lif]sdl: SDL, width: Int32, height: Int32, color: Color
    ) raises:
    ...
    fn fill(self, color: Color, rect: Rect) raises:
        self.sdl[]._sdl.fill_rect(
            self._surface_ptr, adr(rect), color.as_uint32()
        )

    fn fill(self, color: Color) raises:
        self.sdl[]._sdl.fill_rect(
            self._surface_ptr, Ptr[Rect](), color.as_uint32()
        )
    ...
    fn rotozoomed(
        self, angle: Float64, zoom: Float64, smooth: Bool = False
    ) -> Surface[lif]:
        return Surface(
            self.sdl[],
            self.sdl[].gfx.rotozoom_surface(
                self._surface_ptr, angle, zoom, smooth
            ),
        )

    fn convert(inout self, format: Surface):
        self._surface_ptr = self.sdl[]._sdl._convert_surface.call(
            self._surface_ptr, format._surface_ptr[].format, 0
        )
